...
import { 
  ...
  Building2, Phone, UserCheck, MessageSquare, RefreshCw, Loader2, Sparkles, Eye
} from "lucide-react";
...
export default function KitchenManagementPage() {
  ...
  const fetchCenters = async () => {
    try {
      ...
      setCenters(fetched);
      if (fetched.length > 0) {
        setSelectedCenterId(fetched[0].id);
      }
    } catch {
      toast.error("Failed to fetch centers list");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchCenterKitchenData = async (centerId: string, meal: string) => {
    if (!centerId) return;
    setPreviewLoading(true);
    try {
      const [cfgRes, prevRes, logsRes] = await Promise.all([
        api.get(`/cooks/config/${centerId}`).catch(() => null),
        api.get(`/cooks/preview/${centerId}?meal_type=${meal}`).catch(() => null),
        api.get(`/cooks/logs/${centerId}`).catch(() => ({ data: [] }))
      ]);
      ...
      toast.error("Error loading kitchen data");
    } finally {
  ...
  useEffect(() => {
    if (selectedCenterId) {
      fetchCenterKitchenData(selectedCenterId, selectedMeal);
    }
  }, [selectedCenterId, selectedMeal]);
  ...
  const handleTriggerNotification = async () => {
    ...
    try {
      const res = await api.post(`/cooks/trigger/${selectedCenterId}?meal_type=${selectedMeal}`);
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">
              Kitchen &amp; Cook Dining Management
            </h1>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Dynamic Headcount Calculation &amp; Malayalam WhatsApp Alert Dispatcher
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-200">
          ...
          <select
            ...
            className="bg-transparent text-sm font-black text-slate-900 outline-none pr-4 cursor-pointer"
          >
            {centers.map((c) => (
      ...
      {/* ========================================================= */}
      {/* SECTION 1: LIVE DINING HEADCOUNT METRICS & DISPATCH       */}
      {/* ========================================================= */}
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => fetchCenterKitchenData(selectedCenterId, selectedMeal)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-1">
            ...
            <p className="text-[11px] text-emerald-600 font-bold">Exact portions required for {selectedMeal}</p>
          </div>
        ...
        {/* 2-COLUMN GRID: WHATSAPP PREVIEW & COOK PROFILE */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* LEFT 2 COLS: LIVE WHATSAPP MESSAGE PREVIEW */}
          ...
          {/* RIGHT 1 COL: COOK PROFILE SETUP */}
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4 flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <p className="text-xs text-slate-500">
                Registered contact for receiving automatic WhatsApp dining reports for {selectedCenter?.name || "this center"}.
              </p>
      ...
      {/* ========================================================= */}
      {/* SECTION 2: HISTORICAL NOTIFICATION LOGS LEDGER            */}
      {/* ========================================================= */}
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          {logs.length === 0 ? (
            <div className="p-8 text-center text-slate-500 text-sm">
              No cook alert logs recorded yet for this center.
            </div>
          ) : (
