...
import { GraduationCap, UserPlus, Search, Loader2, Mail, Phone, Building2, MapPin, AlertCircle, HeartHandshake, Image as ImageIcon } from "lucide-react";
...
export default function SuperAdminStudentsPage() {
  ...
  const getCenterName = (centerId?: string) => {
    if (!centerId) return "Main Center";
  ...
  return (
    <div className="p-8 space-y-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Manage All Students</h1>
          <p className="text-sm text-slate-500 mt-1">
            Global directory of enrolled students with address, emergency contacts & Gov ID proof.
          </p>
        </div>
        <button
          onClick={() => {
                  ...
                  <tr key={student.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium text-slate-800">
                      <span className="inline-flex items-center gap-1 bg-slate-100 px-2.5 py-1 rounded text-xs">
                        <Building2 className="h-3.5 w-3.5 text-slate-500" /> {getCenterName(student.center_id)}
                      </span>
                    </td>
