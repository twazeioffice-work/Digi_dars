...
import { GraduationCap, UserPlus, Search, Loader2, Mail, Phone, Building2 } from "lucide-react";
...
export default function ManageUstadsPage() {
  ...
  const getCenterName = (centerId?: string) => {
    if (!centerId) return "Main Branch";
  ...
  return (
    <div className="p-8 space-y-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Manage Ustads & Teachers</h1>
          <p className="text-sm text-slate-500 mt-1">
            Audit and register active Quran & Kitab Ustads across Dars centers.
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center text-slate-500 space-y-3">
          ...
          <p className="font-semibold text-lg">No Ustad accounts found</p>
          <p className="text-xs">Click "Register New Ustad" to create a teacher profile.</p>
        </div>
      ) : (
