...
def test_module2_finance_ledger_flow():
    ...
    # 3. Record General Expense -> Expect 201 Created Success
    expense_res = client.post("/api/v1/finance/transactions/expense", json={
        "category_id": zakat_cat_id,
        "amount": 800.00,
        "description": "Boarding ration and stationery",
        "student_id": student_wealthy["id"]
    }, headers=headers_nazim)
    assert expense_res.status_code == 201
    assert expense_res.json()["data"]["amount"] == 800.00

    # 4. Record Expense on Student -> Expect 201 Created Success
    valid_res = client.post("/api/v1/finance/transactions/expense", json={
        "category_id": zakat_cat_id,
        "amount": 1200.00,
        "description": "Purchase of books and uniform for boarding student",
        "receipt_url": "https://storage.provider.com/receipts/exp_4051.pdf",
        "student_id": student_zakat["id"]
    }, headers=headers_nazim)
    assert valid_res.status_code == 201
    tx_data = valid_res.json()["data"]
    assert tx_data["type"] == "DEBIT"
    assert tx_data["amount"] == 1200.00
    tx_id = tx_data["transaction_id"]

    # 5. Reverse Expense Transaction (Immutability pattern)
    reversal_res = client.post(f"/api/v1/finance/transactions/{tx_id}/reverse", json={
    ...
    assert reversal_data["reversal_for_id"] == tx_id
    ...
    assert summary["total_debit"] == 2000.00
    assert summary["net_balance"] == 4200.00
